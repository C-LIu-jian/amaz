                ...
                <table>
                    ...
                    <c:forEach items="${sessionScope.cart.cartItems}" var="CartItem">
                        <tr id="product_id_1">
                            ...
                            <td>
                                <input type="button" value=" - " width="3px" onclick="Num(${CartItem.product.id},${CartItem.quantity-1})">
                                <input type="text" name="number" value="${CartItem.quantity}" style="width:20px;text-align:center;vertical-align:middle"/>
                                <input type="button" value=" + " width="2px" onclick="Num(${CartItem.product.id},${CartItem.quantity+1})"></td>
                             <td>${CartItem.subTotal}</td>
                            <td><a href="#" value="${CartItem.product.id}">删除</a></td>
                        </tr>
                    </c:forEach>
                </table>
			  <div class="button"><a href="${pageContext.request.contextPath}/order.do?param=orderAdd"><input type="button" id="btu"/></a> </div> <%--	 按钮--%>
            </c:otherwise>
        </c:choose>
    </div>
</div>
